...
def extract_features(audio_file):
    ...
    # Extract features (you can customize this based on your needs)
...
def speaker_identifier():
    ...
    file = "Voice-Biometrics\\data\\history\\sample-1_new.webm"
    ...
    mfcc = extract_features(file) #defined in creatingData.py

    input = pd.DataFrame(columns=range(0, 13))

    lst = list(mfcc)
    print("--->",len(lst))
    input.loc[len(input)] = lst

    df = pd.read_csv("Voice-Biometrics/audio_data_new.csv")  # target variable is boolean : 1 means sahil, 0 means unknown
